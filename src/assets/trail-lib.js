...
const GETJSON = function (url, callback) {
  return lion_engine.GETJSON(url);
}
...
// below is depreated.. but here for reference .
const GUID_REPO = "/isis-temp/registration";
function status_dep(guid) {
  request({ 'path': GUID_REPO + '/' + guid + '/' + 'status.json' }).then(re => {
    if (IsJsonString(re)) {
      let js = JSON.parse(re);
      if (js.status) {
        lion_engine.updateProgress(js.status);
      }
    }
    // let j = JSON.parse ( re );
    // sleep ( 1000 );
